...
class Example:
    def __init__(self, viewer, options):
        ...
        # 创建三个物块，位于不同位置
        self.body_block1 = builder.add_body(xform=wp.transform(p=wp.vec3(-0.5, -0.1, drop_z), q=wp.quat_identity()))
        self.body_block2 = builder.add_body(xform=wp.transform(p=wp.vec3(0.0, -0.1, drop_z), q=wp.quat_identity()))
        self.body_block3 = builder.add_body(xform=wp.transform(p=wp.vec3(0.5, -0.1, drop_z), q=wp.quat_identity()))
    ...
    def _classify_by_noise(builder, ids):
        ...
        # 2. 三等分阈值（避免某一类为 0）
        t1 = np.quantile(noise_vals, 0.35)
        t2 = np.quantile(noise_vals, 0.5)
        t3 = np.quantile(noise_vals, 0.9)
