...
class Example:
    def __init__(self, viewer, options):
        ...
        sand_particles, snow_particles, mud_particles = Example.emit_particles(mpm_builder, options)
    ...
    def emit_particles(builder: newton.ModelBuilder, args):
        ...
        all_ids = Example._spawn_particles(builder, args,
                                           particle_lo=np.array([-1.5, -1.5, 0.0]),
                                           particle_hi=np.array([ 1.5,  1.5, 0.3]),
        ...
        sand_ids, snow_ids, mud_ids= Example._classify_by_noise(builder, all_ids)
        ...
        return np.array(sand_ids), np.array(snow_ids), np.array(mud_ids)
    ...
    def _classify_by_noise(builder, ids):
        ...
        t1 = np.quantile(noise_vals, 0.33)
        t2 = np.quantile(noise_vals, 0.66)
...
if __name__ == "__main__":
    ...
    # 沙子相关：粒子生成区域
    parser.add_argument("--emit-lo", type=float, nargs=3, default=[-0.25, -0.25, 0.0])
    parser.add_argument("--emit-hi", type=float, nargs=3, default=[0.25, 0.25, 0.3])
    # 沙子相关：重力设置
...
class TerrainGenerator:
    ...
    def generate_terrain(self, collider_model=None):
        ...
        sand_particles, snow_particles, mud_particles = \
