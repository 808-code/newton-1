...
class Example:
    def __init__(self, viewer, options):
        ...
        builder.add_shape_box(
            thigh, hx=link_hx, hy=link_hy, hz=link_hz, 
            cfg=newton.ModelBuilder.ShapeConfig(density=mass1/(8*link_hx*link_hy*link_hz))
            )

        # 创建小腿
        shank = builder.add_link()
        builder.add_shape_box(
            shank, hx=link_hx, hy=link_hy, hz=link_hz, 
            cfg=newton.ModelBuilder.ShapeConfig(density=mass2/(8*link_hx*link_hy*link_hz))
            )
        # 创建足端
        ...
        # 沙子相关：在指定区域生成 MPM 粒子
        Example.emit_particles(sand_builder, options)
        ...
        # 沙子相关：设置沙子的摩擦属性
        self.sand_model.particle_mu = options.friction_coeff
    ...
    def step(self):
        ...
        force_vec1, force_vec2, force_vec3 = force_vec
        # 这个三维向量的欧几里得范数（也就是它的模长或大小）
        force_magnitude = np.linalg.norm(force_vec3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--critical-fraction", "-cf", type=float, default=0.05) # 临界状态参数

    parser.add_argument("--young-modulus", "-ym", type=float, default=5e7)  # 杨氏模量
    parser.add_argument("--poisson-ratio", "-nu", type=float, default=0.3)     # 泊松比
    parser.add_argument("--friction-coeff", "-mu", type=float, default=0.6)   # 摩擦系数
    parser.add_argument("--damping", type=float, default=5.0)                  # 阻尼
    ...
    parser.add_argument("--yield-stress", "-ys", type=float, default=5.0e4)      # 屈服应力
    parser.add_argument("--hardening", type=float, default=10.0)                # 硬化参数
